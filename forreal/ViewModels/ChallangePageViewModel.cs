...
namespace forreal.ViewModels
{
    public class ChallangePageViewModel : ViewModel
    {
        ...
        public Challanges Challanges { get; set; }
        public List<Challange> ChallangesToday { get; set; }
        #endregion
        #region Proporties
        ...
        {
            _service = service;
            Challanges = new Challanges();
            ChallangesCommand
            for(int i = 1; i < 6; i++)
            {
                var cha = _service.GetChallange(i);
                var ch = cha.Result;
                Challanges.ChallangesList.Add(ch.challange);
            }
            Evt = GetEvent();
            BgColor = "#EB9999";
